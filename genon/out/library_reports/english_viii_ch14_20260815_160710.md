# Library certification · english VIII ch 14 · 20260815_160710

plan: counts [11, 9, 7] · basis authored_standard · registry 6 sections

PASS  library complete: ['ch_14_canonical.json', 'ch_14_canonical_p09.json', 'ch_14_canonical_p07.json'] vs plan [11, 9, 7]
serve granularity: unit  ·  section axis: True
PASS  ch_14_canonical.json: every section the chapter summary carries is anchored by some unit (6 summary section(s) vs 6 registry entr(ies))
PASS  ch_14_canonical.json: standard closes with the mandated `synthesis` unit (and carries the token nowhere else)
PASS  ch_14_canonical.json: every anchor verbatim in the top registry
PASS  ch_14_canonical.json: first-visit order follows the registry
PASS  ch_14_canonical.json: coverage reaches the final registry section before the synthesis unit
PASS  ch_14_canonical_p09.json: the `synthesis` token is reserved to the standard canonical
PASS  ch_14_canonical_p09.json: every anchor verbatim in the top registry
PASS  ch_14_canonical_p09.json: first-visit order follows the registry
PASS  ch_14_canonical_p09.json: coverage reaches the final registry section
PASS  ch_14_canonical_p07.json: the `synthesis` token is reserved to the standard canonical
PASS  ch_14_canonical_p07.json: every anchor verbatim in the top registry
PASS  ch_14_canonical_p07.json: first-visit order follows the registry
PASS  ch_14_canonical_p07.json: coverage reaches the final registry section
PASS  ch_14_canonical.json: register scan reached the band text (44 band(s) read: {'activity_title': 11, 'materials': 16, 'teacher_notes': 11, 'time_bands': 44, 'homework': 1})
PASS  ch_14_canonical.json: register clean (0 ban hit(s))
PASS  ch_14_canonical_p09.json: register scan reached the band text (40 band(s) read: {'activity_title': 9, 'materials': 19, 'teacher_notes': 9, 'time_bands': 40, 'homework': 3})
FAIL  ch_14_canonical_p09.json: register clean (1 ban hit(s))
      ADVISORY ch_14_canonical_p09.json: 1 artefact-dependency hit(s) — a unit reaching for something a PREVIOUS sitting produced. Read them: the brief forbids it, certification cannot.
        U7 time_bands[3] 37–45: 'revise their draft' — Students revise their draft based on the peer response, then two pairs read their conve…
      U6 teacher_notes [forward] …lking distance through a lens—provides a natural conceptual bridge to the space-travel speaking task. Encourage students who are hesi…
      -> declare the fixes in genon/repair_register.py and re-run --certify-only; do NOT hand-edit the artefact
PASS  ch_14_canonical_p07.json: register scan reached the band text (27 band(s) read: {'activity_title': 7, 'materials': 17, 'teacher_notes': 7, 'time_bands': 27, 'homework': 2})
PASS  ch_14_canonical_p07.json: register clean (0 ban hit(s))
PASS  ch_14_canonical.json: every declared stimulus type resolves (0 mis-tagged)
PASS  ch_14_canonical_p09.json: every declared stimulus type resolves (0 mis-tagged)
PASS  ch_14_canonical_p07.json: every declared stimulus type resolves (0 mis-tagged)
PASS  ch_14_canonical.json: every question_type is a known assessment type (0 not)
PASS  ch_14_canonical.json: every non-OPEN_TASK item carries a stem (0 without)
PASS  ch_14_canonical.json: every OPEN_TASK carries an empty stem (0 not)
PASS  ch_14_canonical_p09.json: every question_type is a known assessment type (0 not)
      ADVISORY ch_14_canonical_p09.json: ['ECR'] used by exactly one item in the whole library — check it against the constitution's type table
PASS  ch_14_canonical_p09.json: every non-OPEN_TASK item carries a stem (0 without)
PASS  ch_14_canonical_p09.json: every OPEN_TASK carries an empty stem (0 not)
PASS  ch_14_canonical_p07.json: every question_type is a known assessment type (0 not)
PASS  ch_14_canonical_p07.json: every non-OPEN_TASK item carries a stem (0 without)
PASS  ch_14_canonical_p07.json: every OPEN_TASK carries an empty stem (0 not)
PASS  ch_14_canonical.json: MCQ options in arrangement order
PASS  ch_14_canonical_p09.json: MCQ options in arrangement order
PASS  ch_14_canonical_p07.json: MCQ options in arrangement order

item counts per competency — ADVISORY, does not gate; basis: derived (modal count across this library — no constitution row yet)
      expected {"(from handoff)": 0}
      ch_14_canonical.json: 0 items vs 0 expected
      ch_14_canonical_p09.json: 0 items vs 0 expected
      ch_14_canonical_p07.json: 0 items vs 0 expected
PASS  X=5: choice set non-empty (no defensive truncation)
PASS  X=6: choice set non-empty (no defensive truncation)
PASS  X=7: choice set non-empty (no defensive truncation)
PASS  X=8: choice set non-empty (no defensive truncation)
PASS  X=9: choice set non-empty (no defensive truncation)
PASS  X=10: choice set non-empty (no defensive truncation)
PASS  X=11: choice set non-empty (no defensive truncation)
PASS  X=12: choice set non-empty (no defensive truncation)
PASS  X=13: choice set non-empty (no defensive truncation)

serve sweep: {"5": "fill/single -2s", "6": "fill/single -1s", "7": "identity", "8": "rescue/complete (from 9)", "9": "identity", "10": "fill/single", "11": "identity", "12": "surrender", "13": "surrender"}

options arranged: 7 of 10 item(s) re-ordered this run. Nothing is written to the artefact (founder ruling 2026-08-04), so a 0 here means only that nothing was left to move — on a re-run that is expected, and it is NOT evidence the model arranged them unaided. Read this number on the FIRST pass of a freshly generated library or not at all.
      ch_14_canonical.json: 0 of 3 item(s) re-ordered
      ch_14_canonical_p07.json: 3 of 3 item(s) re-ordered
          #1 None #1: A–D now hold CADB · correct B -> D
          #3 None #3: A–D now hold 1342 · correct 1 -> 1
          #11 None #11: A–D now hold 2413 · correct 1 -> 3
      ch_14_canonical_p09.json: 4 of 4 item(s) re-ordered
          #1 None #1: A–D now hold CBAD · correct B -> B
          #3 None #3: A–D now hold ADBC · correct B -> C
          #4 None #4: A–D now hold 4123 · correct 4 -> 1
          #11 None #11: A–D now hold ADCB · correct A -> A

DETERMINISTIC CHECKS HAVE FAILURES — do not certify.
The HUMAN GATE remains: read the borrowed seams and each closing synthesis in a Cowork session before calling this chapter certified.
