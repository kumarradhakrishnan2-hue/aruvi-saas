# Library certification · mathematics V ch 8 · 20260820_124626

plan: counts [13, 11, 8] · basis authored_standard · registry 14 sections

PASS  library complete: ['ch_08_canonical.json', 'ch_08_canonical_p11.json', 'ch_08_canonical_p08.json'] vs plan [13, 11, 8]
serve granularity: unit  ·  section axis: True
PASS  ch_08_canonical.json: every section the chapter summary carries is anchored by some unit (14 summary section(s) vs 14 registry entr(ies))
PASS  ch_08_canonical.json: standard closes with the mandated `synthesis` unit (and carries the token nowhere else)
PASS  ch_08_canonical.json: every anchor verbatim in the top registry
PASS  ch_08_canonical.json: first-visit order follows the registry
PASS  ch_08_canonical.json: coverage reaches the final registry section before the synthesis unit
PASS  ch_08_canonical_p11.json: the `synthesis` token is reserved to the standard canonical
PASS  ch_08_canonical_p11.json: every anchor verbatim in the top registry
PASS  ch_08_canonical_p11.json: first-visit order follows the registry
PASS  ch_08_canonical_p11.json: coverage reaches the final registry section
PASS  ch_08_canonical_p08.json: the `synthesis` token is reserved to the standard canonical
PASS  ch_08_canonical_p08.json: every anchor verbatim in the top registry
PASS  ch_08_canonical_p08.json: first-visit order follows the registry
PASS  ch_08_canonical_p08.json: coverage reaches the final registry section
PASS  ch_08_canonical.json: register scan reached the band text (39 band(s) read: {'activity_title': 13, 'materials': 29, 'teacher_notes': 13, 'time_bands': 39, 'homework': 4})
PASS  ch_08_canonical.json: register clean (0 ban hit(s))
PASS  ch_08_canonical_p11.json: register scan reached the band text (35 band(s) read: {'activity_title': 11, 'materials': 22, 'teacher_notes': 11, 'time_bands': 35, 'homework': 6})
PASS  ch_08_canonical_p11.json: register clean (0 ban hit(s))
PASS  ch_08_canonical_p08.json: register scan reached the band text (29 band(s) read: {'activity_title': 8, 'materials': 20, 'teacher_notes': 8, 'time_bands': 29, 'homework': 5})
PASS  ch_08_canonical_p08.json: register clean (0 ban hit(s))
PASS  ch_08_canonical.json: every declared stimulus type resolves (0 mis-tagged)
FAIL  ch_08_canonical_p11.json: every declared stimulus type resolves (1 mis-tagged)
      Q-B-6: tagged `number_line:` but has only 2 tick cell(s); at least 3 are needed
      -> either correct the stimulus to satisfy the tag it declares, or drop the tag; a tagged stimulus that fails its contract renders as prose and loses the representation it asked for
PASS  ch_08_canonical_p08.json: every declared stimulus type resolves (0 mis-tagged)
PASS  ch_08_canonical.json: no stem points at a figure the item does not carry (0 does)
PASS  ch_08_canonical_p11.json: no stem points at a figure the item does not carry (0 does)
PASS  ch_08_canonical_p08.json: no stem points at a figure the item does not carry (0 does)
PASS  ch_08_canonical.json: every question_type is a known assessment type (0 not)
PASS  ch_08_canonical.json: every non-OPEN_TASK item carries a stem (0 without)
PASS  ch_08_canonical.json: every OPEN_TASK carries an empty stem (0 not)
PASS  ch_08_canonical_p11.json: every question_type is a known assessment type (0 not)
PASS  ch_08_canonical_p11.json: every non-OPEN_TASK item carries a stem (0 without)
PASS  ch_08_canonical_p11.json: every OPEN_TASK carries an empty stem (0 not)
PASS  ch_08_canonical_p08.json: every question_type is a known assessment type (0 not)
PASS  ch_08_canonical_p08.json: every non-OPEN_TASK item carries a stem (0 without)
PASS  ch_08_canonical_p08.json: every OPEN_TASK carries an empty stem (0 not)
PASS  ch_08_canonical.json: MCQ options in arrangement order
PASS  ch_08_canonical_p11.json: MCQ options in arrangement order
PASS  ch_08_canonical_p08.json: MCQ options in arrangement order

item counts per competency — ADVISORY, does not gate; basis: derived (modal count across this library — no constitution row yet)
      expected {"(from handoff)": 0}
      ch_08_canonical.json: 0 items vs 0 expected
      ch_08_canonical_p11.json: 0 items vs 0 expected
      ch_08_canonical_p08.json: 0 items vs 0 expected
PASS  X=6: choice set non-empty (no defensive truncation)
PASS  X=7: choice set non-empty (no defensive truncation)
PASS  X=8: choice set non-empty (no defensive truncation)
PASS  X=9: choice set non-empty (no defensive truncation)
PASS  X=10: choice set non-empty (no defensive truncation)
PASS  X=11: choice set non-empty (no defensive truncation)
PASS  X=12: choice set non-empty (no defensive truncation)
PASS  X=13: choice set non-empty (no defensive truncation)
PASS  X=14: choice set non-empty (no defensive truncation)
PASS  X=15: choice set non-empty (no defensive truncation)

serve sweep: {"6": "fill/forward -2s", "7": "fill/single -1s", "8": "identity", "9": "rescue/complete (from 11)", "10": "fill/single", "11": "identity", "12": "fill/single", "13": "identity", "14": "surrender", "15": "surrender"}

options arranged: 7 of 21 item(s) re-ordered this run. Nothing is written to the artefact (founder ruling 2026-08-04), so a 0 here means only that nothing was left to move — on a re-run that is expected, and it is NOT evidence the model arranged them unaided. Read this number on the FIRST pass of a freshly generated library or not at all.
      ch_08_canonical.json: 0 of 5 item(s) re-ordered
      ch_08_canonical_p08.json: 0 of 8 item(s) re-ordered
      ch_08_canonical_p11.json: 7 of 8 item(s) re-ordered
          #1 None #1: A–D now hold ABDC · correct B -> B
          #3 None #3: A–D now hold BDCA · correct B -> A
          #4 None #4: A–D now hold CBAD · correct C -> A
          #7 None #7: A–D now hold DCBA · correct B -> C
          #8 None #8: A–D now hold CBAD · correct C -> A
          #9 None #9: A–D now hold BCDA · correct C -> B
          #12 None #12: A–D now hold DCBA · correct C -> B
QUARANTINED  ch_08_canonical_p11.json -> backup/quarantine/mathematics/v/ch_08_canonical_p11_20260820_124626.json

DETERMINISTIC CHECKS HAVE FAILURES — do not certify Failed files are QUARANTINED under backup/quarantine/ (the fix worklist); regenerate them and re-run --certify-only..
The HUMAN GATE remains: read the borrowed seams and each closing synthesis in a Cowork session before calling this chapter certified.
