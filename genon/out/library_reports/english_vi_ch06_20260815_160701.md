# Library certification · english VI ch 6 · 20260815_160701
...
PASS  ch_06_canonical_p10.json: register scan reached the band text (28 band(s) read: {'activity_title': 10, 'materials': 27, 'teacher_notes': 10, 'time_bands': 28, 'homework': 3})
FAIL  ch_06_canonical_p10.json: register clean (2 ban hit(s))
      U3 teacher_notes [forward] …e-analysis and extended-response questions are addressed in the following unit.
      U4 time_bands[2] 28–40 [clock] …nally, and other ways to find real friends — in writing for the remaining time. The three extended responses are completed as homework.
      -> declare the fixes in genon/repair_register.py and re-run --certify-only; do NOT hand-edit the artefact
PASS  ch_06_canonical_p07.json: register scan reached the band text (21 band(s) read: {'activity_title': 7, 'materials': 13, 'teacher_notes': 7, 'time_bands': 21, 'homework': 2})
FAIL  ch_06_canonical_p07.json: register clean (1 ban hit(s))
      U7 teacher_notes [completion] The acrostic poem asks students to draw on vocabulary built across the chapter — qualities like loyal, caring, honest — so the word-buildi…
      -> declare the fixes in genon/repair_register.py and re-run --certify-only; do NOT hand-edit the artefact
...
PASS  ch_06_canonical.json: every question_type is a known assessment type (0 not)
      ADVISORY ch_06_canonical.json: ['FILL_IN'] used by exactly one item in the whole library — check it against the constitution's type table
...
options arranged: 2 of 5 item(s) re-ordered this run. Nothing is written to the artefact (founder ruling 2026-08-04), so a 0 here means only that nothing was left to move — on a re-run that is expected, and it is NOT evidence the model arranged them unaided. Read this number on the FIRST pass of a freshly generated library or not at all.
      ...
      ch_06_canonical_p07.json: 1 of 2 item(s) re-ordered
      ...
      ch_06_canonical_p10.json: 1 of 2 item(s) re-ordered
          #1 None #1: A–D now hold 3214 · correct 3 -> 1
          #3 SKIPPED — more than one correct option on an MCQ — a compound item; arrangement would cross its sub-question boundary; needs a human
