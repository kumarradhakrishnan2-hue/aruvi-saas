...
def briefs_for(subject, klass, chapter):
    ...
    for k in vp["counts"][1:]:
        ...
        lines += [
            ...
            f"- CLOSING MANDATE: the final unit closes the chapter and its "
            f"section_anchor lists exactly the last {span} registry "
            f"section(s): " + " / ".join(tail) + ". This unit must TEACH those "
            "sections — introduce them as new material, do not treat them as "
            "already covered. It may connect back to the chapter's concepts BY "
            "NAME (\"the monsoon mechanism\", \"greenhouse warming\"), but it "
            "MUST NOT assume any earlier unit's activity, reading, discussion "
            "or material actually happened: no \"the case study you read\", no "
            "\"the guidelines we saw\", no \"having traced the full arc\", and "
            "no materials entry belonging to a section this unit does not "
            "anchor. REASON, and it is not stylistic: this is the unit the "
            "serve engine borrows into OTHER plans of this chapter, so it will "
            "be met by classes that never had the units before it. Do NOT add "
            "a further synthesis unit after it — a trailing unit that only "
            "re-anchors sections an earlier unit already taught is never lent, "
            "so the span above would land on the wrong unit. If a coherent "
            "closing arc cannot be built at this span, SAY SO at the end of "
            "the run instead of complying badly.",
