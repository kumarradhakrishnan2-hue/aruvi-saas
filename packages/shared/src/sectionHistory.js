...
function readMap(sectionKey) {
  ...
  try {
    const raw = storage.getItem(historyKey(sectionKey));
    if (!raw) return {};
...
function writeMap(sectionKey, map) {
  try {
    storage.setItem(historyKey(sectionKey), JSON.stringify(map));
...
export function recordHistory(sectionKey, entry) {
  if (!sectionKey || !entry || !entry.file || !entry.status) return;
  const row = {
    file: entry.file,
...
function pushHistory(sectionKey, entries) {
  if (!sectionKey || !entries || !entries.length) return Promise.resolve();
  try {
