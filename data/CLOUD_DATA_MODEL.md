...
| **Ask Aruvi question bank** | `data/cloud/content/ask_aruvi/qa_knowledge_base.json` | Object store | **Added 2026-08-30 — it had never been classified, which is exactly why it went missing.** Founder-authored, hand-written, shared, read-only, served to every signed-in teacher — Bucket A-serve, like `content/legal/`. It lived ONLY inside the web bundle until 2026-08-30, i.e. in a public page chunk readable before sign-in; since it states how period allocation is weighted and how each subject's assessment is built, that was founder IP on a public url. Now served by `GET /ask-aruvi` behind `X-Aruvi-User` and cached in the teacher's localStorage (offline help is the reason it is cached, not bundled). ONE copy on disk — no derived copy to drift. Re-index after editing: `aruvi-scripts/sync_ask_aruvi.py`; guarded by `tests/test_ask_aruvi_kb.py`, whose `test_bank_is_not_bundled` is a security check, not tidiness. |
